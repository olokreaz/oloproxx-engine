...
IOHANDLE io_current_exe( )
{
	...
	if ( GetModuleFileNameW( NULL, wide_path, std::size( wide_path ) ) == 0 || GetLastError( ) != ERROR_SUCCESS ) return 0;
...
IOHANDLE io_open( const char *filename, int flags )
{
	...
	HANDLE handle = CreateFileW( wide_filename.c_str( ),
				     desired_access,
...
int io_seek( IOHANDLE io, int offset, int origin )
{
	...
	switch ( origin ) {
		...
		default		 : dbg_assert( false, "origin invalid" ); return -1;
	}
	...
	while ( true ) {
		...
		if ( aio->read_pos == aio->write_pos ) {
			if ( aio->finish != ASYNCIO_RUNNING ) {
				if ( aio->finish == ASYNCIO_CLOSE ) io_close( aio->io );
		...
		if ( buffers.buf1 ) {
			if ( buffers.len1 > sizeof( local_buffer ) - local_buffer_len ) buffers.len1 = sizeof( local_buffer ) - local_buffer_len;
...
{
	if ( aio->write_pos >= aio->read_pos ) return aio->write_pos - aio->read_pos;
	else return aio->buffer_size + aio->write_pos - aio->read_pos;
}

static unsigned int next_buffer_size( unsigned int cur_size, unsigned int need_size )
{
	while ( cur_size < need_size ) cur_size *= 2;
	return cur_size;
}
...
void aio_wait( ASYNCIO *aio )
{
	void *thread;
	{
		...
		thread	    = aio->thread;
		aio->thread = 0;
		if ( aio->finish == ASYNCIO_RUNNING ) aio->finish = ASYNCIO_EXIT;
	}
	sphore_signal( &aio->sphore );
	thread_wait( thread );
}
...
static_assert( std::chrono::steady_clock::period::den / std::chrono::steady_clock::period::num >= 1000000000,
...
void net_addr_str_v6( const unsigned short ip[ 8 ], int port, char *buffer, int buffer_size )
{
	...
	for ( i = 0; i < 8; i++ ) {
		if ( longest_seq_start <= i && i < longest_seq_start + longest_seq_len ) {
			if ( i == longest_seq_start ) w += str_copy( buffer + w, "::", buffer_size - w );
		} else {
	...
	w += str_copy( buffer + w, "]", buffer_size - w );
	if ( port >= 0 ) str_format( buffer + w, buffer_size - w, ":%d", port );
}
...
void net_addr_str( const NETADDR *addr, char *string, int max_length, int add_port )
{
	if ( addr->type == NETTYPE_IPV4 || addr->type == NETTYPE_WEBSOCKET_IPV4 ) {
		...
		else str_format( string, max_length, "%d.%d.%d.%d", addr->ip[ 0 ], addr->ip[ 1 ], addr->ip[ 2 ], addr->ip[ 3 ] );
	} else if ( addr->type == NETTYPE_IPV6 ) {
		int	       port = -1;
		unsigned short ip[ 8 ];
		int	       i;
		if ( add_port ) port = addr->port;
		for ( i = 0; i < 8; i++ ) ip[ i ] = ( addr->ip[ i * 2 ] << 8 ) | ( addr->ip[ i * 2 + 1 ] );
		net_addr_str_v6( ip, port, string, max_length );
	} else str_format( string, max_length, "unknown type %d", addr->type );
}
...
static int priv_net_extract( const char *hostname, char *host, int max_host, int *port )
{
	...
	*port	  = 0;
	...
	if ( hostname[ 0 ] == '[' ) {
		// ipv6 mode
		for ( i = 1; i < max_host && hostname[ i ] && hostname[ i ] != ']'; i++ ) host[ i - 1 ] = hostname[ i ];
		host[ i - 1 ] = 0;
		if ( hostname[ i ] != ']' )    // malformatted
			return -1;

		i++;
		if ( hostname[ i ] == ':' ) *port = str_toint( hostname + i + 1 );
	} else {
		// generic mode (ipv4, hostname etc)
		for ( i = 0; i < max_host - 1 && hostname[ i ] && hostname[ i ] != ':'; i++ ) host[ i ] = hostname[ i ];
		host[ i ] = 0;
...
int net_host_lookup_impl( const char *hostname, NETADDR *addr, int types )
{
	...
	if ( priv_net_extract( hostname, host, sizeof( host ), &port ) ) return -1;
	...
	if ( types == NETTYPE_IPV4 ) hints.ai_family = AF_INET;
	else if ( types == NETTYPE_IPV6 ) hints.ai_family = AF_INET6;
...
int net_host_lookup( const char *hostname, NETADDR *addr, int types )
{
	...
	if ( ws_hostname ) {
		if ( ( types & NETTYPE_WEBSOCKET_IPV4 ) == 0 ) return -1;
		int result = net_host_lookup_impl( ws_hostname, addr, NETTYPE_IPV4 );
		if ( result == 0 && addr->type == NETTYPE_IPV4 ) addr->type = NETTYPE_WEBSOCKET_IPV4;
		return result;
	}
...
std::string windows_format_system_message( unsigned long error )
{
	...
	if ( FormatMessageW( flags, NULL, error, 0, ( LPWSTR )&wide_message, 0, NULL ) == 0 ) return "unknown error";
...
void fs_listdir( const char *dir, FS_LISTDIR_CALLBACK cb, int type, void *user )
{
	...
	do {
		...
		if ( cb( current_entry.value( ).c_str( ), ( finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) != 0, type, user ) ) break;
	...
	while ( true ) {
		...
		if ( cb( entry->d_name, fs_is_dir( buffer ), type, user ) ) break;
	}
...
void fs_listdir_fileinfo( const char *dir, FS_LISTDIR_CALLBACK_FILEINFO cb, int type, void *user )
{
	...
	do {
		...
		if ( cb( &info, ( finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) != 0, type, user ) ) break;
	} while ( FindNextFileW( handle, &finddata ) );
	...
	while ( true ) {
		...
		if ( cb( &info, fs_is_dir( buffer ), type, user ) ) break;
	}
...
#	else
	if ( str_comp( appname, "Teeworlds" ) == 0 ) {
		...
		} else str_format( path, max, "%s/.local/share/%s", home, appname );
	}
	for ( int i = str_length( path ) - str_length( appname ); path[ i ]; i++ ) path[ i ] = tolower( ( unsigned char )path[ i ] );
#	endif
...
int fs_makedir_rec_for( const char *path )
{
	...
	for ( int index = 1; buffer[ index ] != '\0'; ++index ) {
		...
		if ( ( buffer[ index ] == '/' || buffer[ index ] == '\\' ) && buffer[ index + 1 ] != '\0' && buffer[ index - 1 ] != ':' ) {
			buffer[ index ] = '\0';
			if ( fs_makedir( buffer ) < 0 ) return -1;
			buffer[ index ] = '/';
		}
	}
	return 0;
...
int fs_makedir( const char *path )
{
	...
	if ( CreateDirectoryW( wide_path.c_str( ), NULL ) != 0 ) return 0;
...
#if defined( CONF_FAMILY_WINDOWS )
	const std::wstring wide_path = windows_utf8_to_wide( path );
	if ( RemoveDirectoryW( wide_path.c_str( ) ) != 0 ) return 0;
	return -1;
#else
...
const char *fs_filename( const char *path )
{
	for ( const char *filename = path + str_length( path ); filename >= path; --filename )
		if ( filename[ 0 ] == '/' || filename[ 0 ] == '\\' ) return filename + 1;
	return path;
}
...
void fs_split_file_extension( const char *filename, char *name, size_t name_size, char *extension, size_t extension_size )
{
	...
	if ( last_dot == nullptr || last_dot == filename ) {
		...
		if ( name != nullptr ) str_copy( name, filename, name_size );
	} else {
		if ( extension != nullptr ) str_copy( extension, last_dot + 1, extension_size );
		if ( name != nullptr ) str_truncate( name, name_size, filename, last_dot - filename );
	}
}
...
int fs_rename( const char *oldname, const char *newname )
{
	...
	if ( MoveFileExW( wide_oldname.c_str( ), wide_newname.c_str( ), MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED ) == 0 ) return 1;
#else
...
int net_socket_read_wait( NETSOCKET sock, int time )
{
	...
	if ( sock->ipv6sock >= 0 ) {
		...
		if ( sock->ipv6sock > sockid ) sockid = sock->ipv6sock;
	}
	...
	if ( time < 0 ) select( sockid + 1, &readfds, NULL, NULL, NULL );
	else select( sockid + 1, &readfds, NULL, NULL, &tv );

	if ( sock->ipv4sock >= 0 && FD_ISSET( sock->ipv4sock, &readfds ) ) return 1;
#if defined( CONF_WEBSOCKETS )
	if ( sock->web_ipv4sock >= 0 && FD_ISSET( sockid, &readfds ) ) return 1;
#endif
	if ( sock->ipv6sock >= 0 && FD_ISSET( sock->ipv6sock, &readfds ) ) return 1;
...
static bool time_iseasterday( time_t time_data, struct tm *time_info )
{
	...
	for ( int day_offset = -1; day_offset <= 2; day_offset++ ) {
		...
		if ( time_info->tm_mon == month - 1 && time_info->tm_mday == day ) return true;
	}
	return false;
}
...
ETimeSeason time_season( )
{
	...
	if ( ( time_info->tm_mon == 11 && time_info->tm_mday == 31 ) || ( time_info->tm_mon == 0 && time_info->tm_mday == 1 ) ) return SEASON_NEWYEAR;
	else if ( time_info->tm_mon == 11 && time_info->tm_mday >= 24 && time_info->tm_mday <= 26 ) return SEASON_XMAS;
	else if ( ( time_info->tm_mon == 9 && time_info->tm_mday == 31 ) || ( time_info->tm_mon == 10 && time_info->tm_mday == 1 ) )
		return SEASON_HALLOWEEN;
	else if ( time_iseasterday( time_data, time_info ) ) return SEASON_EASTER;
	...
	switch ( time_info->tm_mon ) {
		...
		default: dbg_assert( false, "Invalid month" ); dbg_break( );
	}
}
...
void str_utf8_truncate( char *dst, int dst_size, const char *src, int truncation_len )
{
	...
	while ( pos <= truncation_len && cursor - src < dst_size && size != cursor - src ) {
		size = cursor - src;
		if ( str_utf8_decode( &cursor ) == 0 ) break;
		pos++;
	}
...
{
	int size = dst_size;
	if ( truncation_len < size ) size = truncation_len + 1;
...
{
	while ( *str && str_isspace( *str ) ) str++;
	while ( words && *str ) {
		if ( str_isspace( *str ) && !str_isspace( *( str + 1 ) ) ) words--;
		str++;
	}
	return str;
}
...
int str_comp_filenames( const char *a, const char *b )
{
	...
	for ( ; *a && *b; ++a, ++b ) {
		if ( str_isnum( *a ) && str_isnum( *b ) ) {
			...
			if ( str_isnum( *a ) ) return 1;
			else if ( str_isnum( *b ) ) return -1;
			else if ( result || *a == '\0' || *b == '\0' ) return result;
		}
...
const char *str_startswith_nocase( const char *str, const char *prefix )
{
	...
	if ( str_comp_nocase_num( str, prefix, prefixl ) == 0 ) return str + prefixl;
	else return 0;
}
...
{
	int prefixl = str_length( prefix );
	if ( str_comp_num( str, prefix, prefixl ) == 0 ) return str + prefixl;
	else return 0;
}
...
const char *str_endswith_nocase( const char *str, const char *suffix )
{
	...
	if ( strl < suffixl ) return 0;
	strsuffix = str + strl - suffixl;
	if ( str_comp_nocase( strsuffix, suffix ) == 0 ) return strsuffix;
	else return 0;
}
...
const char *str_endswith( const char *str, const char *suffix )
{
	...
	if ( strl < suffixl ) return 0;
	strsuffix = str + strl - suffixl;
	if ( str_comp( strsuffix, suffix ) == 0 ) return strsuffix;
	else return 0;
}
...
#define B( i, j ) buf[ ( ( j ) & 1 ) * ( a_len + 1 ) + ( i ) ]
	for ( i = 0; i <= a_len; i++ ) B( i, 0 ) = i;
	for ( j = 1; j <= b_len; j++ ) {
		B( 0, j ) = j;
		for ( i = 1; i <= a_len; i++ ) {
			int subst = ( a[ i - 1 ] != b[ j - 1 ] );
			B( i, j ) = min3( B( i - 1, j ) + 1, B( i, j - 1 ) + 1, B( i - 1, j - 1 ) + subst );
		}
	}
...
bool str_delimiters_around_offset( const char *haystack, const char *delim, int offset, int *start, int *end )
{
	...
	*start		      = 0;
	while ( str_find( search, delim ) ) {
		...
		if ( distance > offset ) break;

		*start = distance;
		search = test;
	}
	if ( search == haystack ) found = false;
...
void str_hex_cstyle( char *dst, int dst_size, const void *data, int data_size, int bytes_per_line )
{
	...
	if ( dst_index >= 1 ) dst[ dst_index - 1 ] = '\0';
	if ( dst_index >= 2 ) dst[ dst_index - 2 ] = '\0';
}
...
int str_hex_decode( void *dst, int dst_size, const char *src )
{
	...
	for ( i = 0; i < len && dst_size; i++, dst_size-- )
		if ( byteval( src + i * 2, cdst++ ) ) return 1;
	return 0;
}
...
void str_base64( char *dst, int dst_size, const void *data_raw, int data_size )
{
	...
	while ( true ) {
		...
		if ( o == dst_size ) return;
		if ( num_bits > 0 ) {
			unsigned padded;
			if ( num_bits >= 6 ) padded = ( value >> ( num_bits - 6 ) ) & 0x3f;
			else padded = ( value << ( 6 - num_bits ) ) & 0x3f;
			dst[ o ] = DIGITS[ padded ];
			num_bits -= 6;
			o += 1;
		} else if ( o % 4 != 0 ) {
...
static int base64_digit_value( char digit )
{
	...
	else if ( 'a' <= digit && digit <= 'z' ) return digit - 'a' + 26;
	else if ( '0' <= digit && digit <= '9' ) return digit - '0' + 52;
		...
		case TIME_HOURS:
			if ( centisecs >= hour )
				return str_format( buffer,
						   ...
						   ( centisecs % min ) / sec );
			[[fallthrough]];
		case TIME_MINS: return str_format( buffer, buffer_size, "%02" PRId64 ":%02" PRId64, centisecs / min, ( centisecs % min ) / sec );
		case TIME_HOURS_CENTISECS:
			if ( centisecs >= hour )
				return str_format( buffer,
						   buffer_size,
						   "%02" PRId64 ":%02" PRId64 ":%02" PRId64 ".%02" PRId64,
						   centisecs / hour,
						   ( centisecs % hour ) / min,
						   ( centisecs % min ) / sec,
						   centisecs % sec );
...
{
	while ( *str ) {
		if ( !str_isnum( *str ) && !( *str >= 'a' && *str <= 'f' ) && !( *str >= 'A' && *str <= 'F' ) ) return 0;
		str++;
	}
	return 1;
}
...
{
	return code <= 0x0020 || code == 0x0085 || code == 0x00A0 || code == 0x034F || code == 0x115F || code == 0x1160 || code == 0x1680
	       || code == 0x180E || ( code >= 0x2000 && code <= 0x200F ) || ( code >= 0x2028 && code <= 0x202F )
	       || ( code >= 0x205F && code <= 0x2064 ) || ( code >= 0x206A && code <= 0x206F ) || code == 0x2800 || code == 0x3000 || code == 0x3164
	       || ( code >= 0xFE00 && code <= 0xFE0F ) || code == 0xFEFF || code == 0xFFA0 || ( code >= 0xFFF9 && code <= 0xFFFC );
}
...
const char *str_utf8_skip_whitespaces( const char *str )
{
	...
	while ( *str ) {
		...
		if ( !str_utf8_isspace( code ) ) return str_old;
	}
...
void str_utf8_trim_right( char *param )
{
	...
	while ( *str ) {
		...
		// check if unicode is not empty
		if ( !str_utf8_isspace( code ) ) end = 0;
		else if ( !end ) end = str_old;
	}
	if ( end ) *end = 0;
}
...
int str_utf8_rewind( const char *str, int cursor )
{
	while ( cursor ) {
		cursor--;
		if ( str_utf8_isstart( *( str + cursor ) ) ) break;
	}
	return cursor;
}
...
int str_utf8_forward( const char *str, int cursor )
{
	...
	if ( str_utf8_decode( &ptr ) == 0 ) return cursor;
	return ptr - str;
}
	...
	while ( true ) {
		...
		if ( utf8_bytes_needed == 0 ) {
			if ( byte_value <= 0x7F ) {
				...
				if ( byte_value == 0xE0 ) utf8_lower_boundary = 0xA0;
				if ( byte_value == 0xED ) utf8_upper_boundary = 0x9F;
				utf8_bytes_needed = 2;
				utf8_code_point	  = byte_value - 0xE0;
			} else if ( 0xF0 <= byte_value && byte_value <= 0xF4 ) {
				if ( byte_value == 0xF0 ) utf8_lower_boundary = 0x90;
				if ( byte_value == 0xF4 ) utf8_upper_boundary = 0x8F;
				utf8_bytes_needed = 3;
		...
		if ( utf8_bytes_seen != utf8_bytes_needed ) continue;
...
void str_utf8_stats( const char *str, size_t max_size, size_t max_count, size_t *size, size_t *count )
{
	...
	*count		   = 0;
	while ( *size < max_size && *count < max_count ) {
		if ( str_utf8_decode( &cursor ) == 0 ) break;
		if ( ( size_t )( cursor - str ) >= max_size ) break;
		*size = cursor - str;
		++( *count );
	}
}
...
size_t str_utf8_offset_bytes_to_chars( const char *str, size_t byte_offset )
{
	...
	while ( current_offset < byte_offset ) {
		...
		if ( current_offset == prev_byte_offset ) break;
		char_offset++;
	}
	return char_offset;
}
...
size_t str_utf8_offset_chars_to_bytes( const char *str, size_t char_offset )
{
	...
	for ( size_t i = 0; i < char_offset; i++ ) {
		...
		if ( byte_offset == prev_byte_offset ) break;
	}
	return byte_offset;
}
...
{
	unsigned hash = 5381;
	for ( ; *str; str++ ) hash = ( ( hash << 5 ) + hash ) + ( *str ); /* hash * 33 + c */
	return hash;
}
...
#	if defined( CONF_FAMILY_WINDOWS )
	...
	switch ( window_state ) {
		case EShellExecuteWindowState::FOREGROUND: info.nShow = SW_SHOW; break;
		case EShellExecuteWindowState::BACKGROUND: info.nShow = SW_SHOWMINNOACTIVE; break;
		default					 : dbg_assert( false, "window_state invalid" ); dbg_break( );
	}
	...
	if ( fegetenv_result == 0 ) fesetenv( &floating_point_environment );
...
#	if defined( CONF_FAMILY_WINDOWS )
	...
	return false;
#	elif defined( CONF_FAMILY_UNIX )
	if ( !is_process_alive( process ) ) return true;
	int status;
...
{
	if ( process == INVALID_PROCESS ) return false;
#	if defined( CONF_FAMILY_WINDOWS )
	DWORD exit_code;
...
#	elif defined( CONF_PLATFORM_LINUX )
	const int pid = fork( );
	if ( pid == 0 ) execlp( "xdg-open", "xdg-open", link, nullptr );
	return pid > 0;
#	elif defined( CONF_FAMILY_UNIX )
	const int pid = fork( );
	if ( pid == 0 ) execlp( "open", "open", link, nullptr );
	return pid > 0;
#	endif
}
...
#	else
	...
	if ( fs_is_relative_path( path ) ) {
		if ( !fs_getcwd( workingDir, sizeof( workingDir ) ) ) return 0;
...
void os_locale_str( char *locale, size_t length )
{
	...
	dbg_assert( CFStringGetCString( locale_identifier_ref, locale_identifier, locale_identifier_size, kCFStringEncodingUTF8 ),
		    "CFStringGetCString failure" );
	...
	for ( int i = 0; i < str_length( locale ); ++i ) {
		if ( locale[ i ] == '_' ) {
			locale[ i ] = '-';
		} else if ( locale[ i ] != '-' && !( locale[ i ] >= 'a' && locale[ i ] <= 'z' ) && !( locale[ i ] >= 'A' && locale[ i ] <= 'Z' )
			    && !( str_isnum( locale[ i ] ) ) ) {
			locale[ i ] = '\0';
			break;
		}
	...
	if ( locale[ 0 ] == '\0' || str_comp( locale, "C" ) == 0 || str_comp( locale, "POSIX" ) == 0 ) str_copy( locale, "en-US", length );
}
...
#	if defined( CONF_FAMILY_WINDOWS )
	...
	if ( exception_handling_module == nullptr ) {
		const DWORD	  LastError = GetLastError( );
		const std::string ErrorMsg  = windows_format_system_message( LastError );
		dbg_msg( "exception_handling",
			 "failed to load exception handling library '%s' (error %ld %s)",
			 module_name,
			 LastError,
			 ErrorMsg.c_str( ) );
	}
...
#		endif
		...
		auto	    exception_log_file_path_func
				= ( BOOL( APIENTRY * )( const WCHAR * ) )( GetProcAddress( exception_handling_module, function_name ) );
#		ifdef __MINGW32__
#			pragma GCC diagnostic pop
#		endif
		if ( exception_log_file_path_func == nullptr ) {
			...
			const std::string ErrorMsg  = windows_format_system_message( LastError );
			dbg_msg( "exception_handling",
				 "could not find function '%s' in exception handling library (error %ld %s)",
				 function_name,
				 LastError,
				 ErrorMsg.c_str( ) );
		} else exception_log_file_path_func( wide_log_file_path.c_str( ) );
	}
#	else
#		error exception handling not implemented
#	endif
...
std::wstring windows_utf8_to_wide( const char *str )
{
	...
	dbg_assert( MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, str, orig_length, wide_string.data( ), size_needed ) == size_needed,
...
std::optional< std::string > windows_wide_to_utf8( const wchar_t *wide_str )
{
	...
	dbg_assert( WideCharToMultiByte( CP_UTF8, WC_ERR_INVALID_CHARS, wide_str, orig_length, string.data( ), size_needed, nullptr, nullptr )
				    == size_needed,
...
bool shell_register_protocol( const char *protocol_name, const char *executable, bool *updated )
{
	...
	const LRESULT result_subkey_protocol = RegCreateKeyExW( handle_subkey_classes,
								protocol_name_wide.c_str( ),
								0,
								NULL,
								0,
								KEY_ALL_ACCESS,
								NULL,
								&handle_subkey_protocol,
								NULL );
	...
	const LRESULT	   result_value_protocol = RegSetValueExW( handle_subkey_protocol,
								   L"",
								   0,
								   REG_SZ,
								   ( BYTE      *)value_protocol.c_str( ),
								   ( value_protocol.length( ) + 1 ) * sizeof( wchar_t ) );
	...
	const LRESULT result_subkey_icon
			= RegCreateKeyExW( handle_subkey_protocol, L"DefaultIcon", 0, NULL, 0, KEY_ALL_ACCESS, NULL, &handle_subkey_icon, NULL );
	...
	const LRESULT	   result_value_icon = RegSetValueExW( handle_subkey_icon,
							       L"",
							       0,
							       REG_SZ,
							       ( BYTE	   *)value_icon.c_str( ),
							       ( value_icon.length( ) + 1 ) * sizeof( wchar_t ) );
	...
	const LRESULT result_subkey_shell_open_command = RegCreateKeyExW( handle_subkey_protocol,
									  ...
									  KEY_ALL_ACCESS,
									  NULL,
									  &handle_subkey_shell_open_command,
									  NULL );
	RegCloseKey( handle_subkey_protocol );
	...
	const LRESULT	   result_old_value_executable = RegGetValueW( handle_subkey_shell_open_command,
								       NULL,
								       L"",
								       RRF_RT_REG_SZ,
								       NULL,
								       ( BYTE	   *)old_value_executable,
								       &old_size_executable );
	...
	if ( result_old_value_executable != ERROR_SUCCESS || wcscmp( old_value_executable, value_executable.c_str( ) ) != 0 ) {
		...
		const LRESULT result_value_executable = RegSetValueExW( handle_subkey_shell_open_command,
									L"",
									0,
									REG_SZ,
									( BYTE * )value_executable.c_str( ),
									( value_executable.length( ) + 1 ) * sizeof( wchar_t ) );
...
bool shell_register_extension( const char *extension, const char *description, const char *executable_name, const char *executable, bool *updated )
{
	...
	const LRESULT result_subkey_program_id = RegCreateKeyExW( handle_subkey_classes,
								  program_id_wide.c_str( ),
								  0,
								  NULL,
								  0,
								  KEY_ALL_ACCESS,
	...
	const LRESULT result_description_default = RegSetValueExW( handle_subkey_program_id,
								   L"",
								   0,
								   REG_SZ,
								   ( BYTE * )description_wide.c_str( ),
								   ( description_wide.length( ) + 1 ) * sizeof( wchar_t ) );
	...
	const LRESULT result_description_friendly = RegSetValueExW( handle_subkey_program_id,
								    L"FriendlyTypeName",
								    0,
								    REG_SZ,
								    ( BYTE * )description_wide.c_str( ),
	...
	HKEY	      handle_subkey_icon;
	const LRESULT result_subkey_icon
			= RegCreateKeyExW( handle_subkey_program_id, L"DefaultIcon", 0, NULL, 0, KEY_ALL_ACCESS, NULL, &handle_subkey_icon, NULL );
	if ( result_subkey_icon != ERROR_SUCCESS ) {
	...
	// Create the "shell\open\command" subkeys
	HKEY	      handle_subkey_shell_open_command;
	const LRESULT result_subkey_shell_open_command = RegCreateKeyExW( handle_subkey_program_id,
									  L"shell\\open\\command",
									  0,
									  NULL,
									  0,
	...
	const LRESULT result_subkey_extension = RegCreateKeyExW( handle_subkey_classes,
								 extension_wide.c_str( ),
								 0,
								 NULL,
								 0,
								 KEY_ALL_ACCESS,
								 NULL,
								 &handle_subkey_extension,
								 NULL );
	RegCloseKey( handle_subkey_classes );
	...
	const LRESULT result_old_value_application = RegGetValueW( handle_subkey_extension,
								   NULL,
								   L"",
								   RRF_RT_REG_SZ,
								   NULL,
								   ( BYTE * )old_value_application,
								   &old_size_application );
	if ( result_old_value_application != ERROR_SUCCESS || wcscmp( old_value_application, program_id_wide.c_str( ) ) != 0 ) {
		...
		const LRESULT result_value_application = RegSetValueExW( handle_subkey_extension,
									 L"",
									 0,
									 REG_SZ,
									 ( BYTE * )program_id_wide.c_str( ),
									 ( program_id_wide.length( ) + 1 ) * sizeof( wchar_t ) );
...
bool shell_register_application( const char *name, const char *executable, bool *updated )
{
	...
	const LRESULT result_subkey_applications
			= RegOpenKeyExW( HKEY_CURRENT_USER, L"SOFTWARE\\Classes\\Applications", 0, KEY_ALL_ACCESS, &handle_subkey_applications );
	...
	const LRESULT result_subkey_program = RegCreateKeyExW( handle_subkey_applications,
	...
	DWORD	      old_size_executable	  = sizeof( old_value_executable );
	const LRESULT result_old_value_executable = RegGetValueW( handle_subkey_program,
								  NULL,
								  L"FriendlyAppName",
								  RRF_RT_REG_SZ,
								  NULL,
	...
	if ( result_old_value_executable != ERROR_SUCCESS || wcscmp( old_value_executable, name_wide.c_str( ) ) != 0 ) {
		...
		const LRESULT result_program_name = RegSetValueExW( handle_subkey_program,
								    L"FriendlyAppName",
								    0,
								    REG_SZ,
								    ( BYTE * )name_wide.c_str( ),
								    ( name_wide.length( ) + 1 ) * sizeof( wchar_t ) );
